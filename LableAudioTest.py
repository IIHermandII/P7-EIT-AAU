...
def MarkSegment2(AudioDataVector, SampleRate, Model, track, WindowLength=0.2, OverLab=0.0): # sr = 1 sec = 44100
    #WL and HL is in sec - then convert to samples
    #############################################
    #       VERSION 1 large Voice               #
    #                                           #
...
def mute_segments(y, sr, model, track, WL=0.2, OL=0): # sr = 1 sec = 44100
    #WL and HL is in sec - then convert to samples
...
def SoundFileFromLableFile(LableFileLOcation):
    print("Working dir: ")
    print(LableFileLOcation)
    y, sr = GetDataFiles("2.wav")
    print(len(y))
    print(sr)
    print(len(y)/(sr*60))
    muted_audio = np.copy(y)
    end_gain = 0
    f = open("C:\\Users\\emill\\OneDrive - Aalborg Universitet\\GIT\\P7-EIT-AAU-DIFRENT-BRANCHES\\JensUlrik\\P7-EIT-AAU\\Outputs\\Predictions_2_Smart(SVM).txt", "r")
    ...
    sf.write("C:\\Users\\emill\\OneDrive - Aalborg Universitet\\P7\\Data\\Refined data\\smart62.wav", muted_audio, sr)
...
def main():
    ...
    # print("Model loaded")    
    # SmartPredictionList = MarkSegment2(y,sr,pipe,"2")
    # f = open("Outputs\\Predictions_2_Smart(SVM).txt", "w")
    # for pred in SmartPredictionList:
    #     f.write(pred + "\n")
    # f.close()

    # OrignPredictionList = mute_segments(y,sr,pipe,"2")
    # f = open("Outputs\\Predictions_2_Origin(SVM).txt", "w")
    # for pred in OrignPredictionList:
    #     f.write(pred + "\n")
    # f.close()

    SoundFileFromLableFile("Outputs\\Predictions_2_Smart(SVM).txt")
